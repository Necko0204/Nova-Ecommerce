  ...
  template: `
    <div class="admin-shell" [class.admin-shell--collapsed]="collapsed()">
      ...
      <div class="admin-main">
        <header class="admin-topbar"><button class="mobile-menu" (click)="mobileOpen.set(true)" aria-label="Open navigation"><lucide-icon [img]="Menu" [size]="19" /></button><div><p>{{ breadcrumb() }}</p><h1>{{ pageTitle() }}</h1></div><div class="topbar-actions"><button class="command-trigger" (click)="paletteOpen.set(true)"><lucide-icon [img]="Search" [size]="15" /><span>Search or jump to…</span><kbd>⌘ K</kbd></button><div class="merchant-menu-wrap" (click)="$event.stopPropagation()"><button class="merchant-menu" (click)="merchantOpen.set(!merchantOpen())" aria-haspopup="menu" [attr.aria-expanded]="merchantOpen()"><span>MR</span><div><strong>{{ auth.user()?.fullName || 'Administrator' }}</strong><small>Administrator</small></div><lucide-icon [img]="ChevronDown" [size]="14" /></button>@if (merchantOpen()) {<div class="merchant-dropdown" role="menu"><div class="merchant-dropdown__identity"><strong>{{ auth.user()?.fullName || 'Administrator' }}</strong><span>{{ auth.user()?.email }}</span></div><a routerLink="/admin/settings" role="menuitem" (click)="merchantOpen.set(false)"><lucide-icon [img]="Settings" [size]="15" />Settings</a><button type="button" role="menuitem" (click)="signOut()"><lucide-icon [img]="LogOut" [size]="15" />Sign out</button></div>}</div></div></header>
...
export class AdminLayoutComponent {
  ...
  readonly collapsed = signal(false); readonly mobileOpen = signal(false); readonly paletteOpen = signal(false); readonly merchantOpen = signal(false); readonly query = signal(''); readonly currentUrl = signal(this.router.url);
  ...
  @HostListener('document:click') onDocumentClick() { this.merchantOpen.set(false); }
  @HostListener('document:keydown', ['$event']) onKey(event: KeyboardEvent) { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); this.paletteOpen.set(!this.paletteOpen()); } if (event.key === 'Escape') { this.paletteOpen.set(false); this.merchantOpen.set(false); } }
  async runCommand(item: CommandItem) { this.paletteOpen.set(false); this.query.set(''); await this.router.navigateByUrl(item.route); }
  async signOut() { this.merchantOpen.set(false); await this.auth.signOut(); await this.router.navigateByUrl('/login'); }
}
