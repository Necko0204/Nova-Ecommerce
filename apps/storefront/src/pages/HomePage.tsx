...
export function HomePage() {
  ...
  const fallbackHeroImage = products[0]?.images[0]?.url ?? '';
  const fallbackEditorialImage = products[1]?.images[0]?.url ?? fallbackHeroImage;
  const heroImage = heroProduct?.images[0]?.url ?? fallbackHeroImage;
  const editorialImage = editorialProduct?.images[1]?.url ?? editorialProduct?.images[0]?.url ?? fallbackEditorialImage;
  ...
  return (
    <main>
      <section className="hero">
        <div className="hero__media"><img src={heroImage} alt="Nova Carry Backpack prepared for a day in motion" onError={({ currentTarget }) => { currentTarget.onerror = null; currentTarget.src = fallbackHeroImage; }} /></div>
      ...
      <section className="editorial page-shell">
        <motion.div {...reveal} className="editorial__image"><img src={editorialImage} alt="Aero Wireless Headphones in a quiet listening space" onError={({ currentTarget }) => { currentTarget.onerror = null; currentTarget.src = fallbackEditorialImage; }} /><span className="editorial__label">Edition N° 04</span></motion.div>
